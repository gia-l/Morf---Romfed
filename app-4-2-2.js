(function(){
  ...
  function isDictionarySegment(seg){
    ...
    return seg && (source === 'lexicon' || source === 'vocabulary' || source === 'name' || source === 'variation' || source === 'family');
  }
  ...
  function displaySegmentsForResult(item){
    ...
    const rawSegs = Array.isArray(item && item.displaySegs) ? item.displaySegs : (Array.isArray(item && item.segs) ? item.segs : []);
    ...
    wrap.innerHTML = results.map((item, idx) => {
      ...
      return `<article class="resultItem${item.picked ? ' picked' : ''}" data-word="${escapeHtml(item.word)}">
        ...
        <div class="resultAddButtons">
          <button class="small quickAddWord primary" type="button" data-prefer="" data-form="${escapeHtml(item.word)}" data-gloss="${quickGloss}">Add to…</button>
        </div>
      </article>`;
    }).join('');
...
Jord[a/y]n, Jordy = river child">${escapeHtml(M.nameEntriesToText ? M.nameEntriesToText(c.entries || []) : '')}</textarea></label>
  ...
  function openSegmentDialog(seg, prefer=''){
    ...
    $('#segmentDialogTitle').textContent = prefer === 'lex' ? 'Add to Lexicon' : (prefer === 'vocab' ? 'Add to Vocabulary' : (prefer === 'name' ? 'Add to Names' : 'Add to…'));
