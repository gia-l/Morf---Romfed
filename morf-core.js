(function(root){
  ...
  function isGeneratedBoundarySegment(seg){
    const source = String((seg && seg.source) || '').toLowerCase();
    return source === 'lexicon' || source === 'vocabulary' || source === 'name' || source === 'variation' || source === 'family';
  }

  function findFoldedIndex(haystack, needle, start){
    const h = String(haystack || '');
    const n = String(needle || '');
    if(!n) return -1;
    const idx = h.toLocaleLowerCase().indexOf(n.toLocaleLowerCase(), Math.max(0, start || 0));
    return idx;
  }

  function collapseGeneratedOutputSegments(word, segs){
    const finalWord = String(word || '');
    const rawSegs = Array.isArray(segs) ? segs : [];
    if(!finalWord) return [];
    if(!rawSegs.length) return [{ form: finalWord, cat: 'generated', source: 'generated', gloss: '?' }];

    const aligned = [];
    let pos = 0;
    for(const seg of rawSegs){
      const rawForm = String((seg && seg.form) || '');
      if(!rawForm) continue;
      const idx = findFoldedIndex(finalWord, rawForm, pos);
      if(idx < 0) continue;
      if(idx > pos){
        aligned.push({ form: finalWord.slice(pos, idx), cat: 'generated', source: 'generated', gloss: '?' });
      }
      const actualForm = finalWord.slice(idx, idx + rawForm.length);
      if(isGeneratedBoundarySegment(seg)){
        aligned.push(Object.assign({}, seg, { form: actualForm }));
      } else {
        aligned.push({ form: actualForm, cat: 'generated', source: 'generated', gloss: '?' });
      }
      pos = idx + rawForm.length;
      if(pos >= finalWord.length) break;
    }
    if(pos < finalWord.length){
      aligned.push({ form: finalWord.slice(pos), cat: 'generated', source: 'generated', gloss: '?' });
    }
    ...
    const flush = () => {
      if(unknown){
        out.push({ form: unknown, cat: 'generated', source: 'generated', gloss: '?' });
        unknown = '';
      }
    };
    for(const seg of aligned){
      if(isGeneratedBoundarySegment(seg)){
        ...
        unknown += String((seg && seg.form) || '');
      }
    }
    flush();
    return out.length ? out : [{ form: finalWord, cat: 'generated', source: 'generated', gloss: '?' }];
  }
  ...
  function generateWords(state, options={}){
    ...
    while(results.length < target && stats.attempts < maxAttempts){
      ...
      const sourceSegs = Array.isArray(built.segs) ? built.segs : [];
      try {
        ...
        const segs = collapseGeneratedOutputSegments(word, sourceSegs);
        ...
        results.push({ word, gloss, segs, rawSegs: sourceSegs, analysis, rewrites: rewritten.fired, adjustments: adjusted.changed });
      } catch(err){
        stats.failed++;
        if(stats.errors.length < 10) stats.errors.push(err.message || String(err));
      }
  ...
  const api = {
    ...
    analyzeNameLiteral,
    glossForSegments,
    collapseGeneratedOutputSegments,
    parseRewriteRules,
    applyRewriteRules,
    parseForbiddenRules,
    violatesForbidden,
