(function(root){
  ...
  function parseEntryLineMany(line, kind='lex'){
    ...
    return forms.map(form => kind === 'vocab'
      ? { id: uid('ve'), word: form, gloss: right }
      : { id: uid('le'), form, gloss: right });
  }
  ...
  function parseNameEntryLineMany(line){
    ...
    for(const unit of units){
      ...
      out.push({ id: uid('ne'), name: main, actual, literal, notes, nicknames });
    }
    return out;
  }
    ...
    return list.map(c => {
      ...
      return {
        ...
        entries: rawEntries.flatMap(e => {
          ...
          const gloss = mapStyle ? cleanString(e.value, '') : cleanString(e && (e.gloss ?? e.meaning ?? e.label ?? e.definition ?? ''), '');
  ...
  function normalizeVocabularyCompat(src, baseCategories){
    ...
    const looseEntries = ensureArrayish(looseRaw).flatMap(e => {
      ...
      const gloss = cleanString(e && (e.gloss ?? e.meaning ?? e.label ?? e.definition ?? ''), '');
      return splitEntryForms(first).map((word, idx) => ({
        id: (idx === 0 && e && e.id) ? e.id : uid('ve'),
        word,
        gloss
