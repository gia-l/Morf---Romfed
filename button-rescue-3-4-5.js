/* Morf 3.4.5 pre-app button rescue.
...
(function(){
  ...
  var STORE_KEY = 'morf-3-4-5-compat-settings';
  var lastResults = [];
  var lastStats = {};
  ...
  function renderResults(results, stats, st){
    ...
    wrap.innerHTML = results.map(function(item, i){
      return '<article class="resultItem'+(item.picked?' picked':'')+'"><div class="resultTop"><span class="num">'+(i+1)+'</span><strong class="word">'+esc(item.word)+'</strong>'+(item.picked?'<span class="tag pickedTag">random pick</span>':'')+(item.gloss?' <span class="gloss">'+esc(item.gloss)+'</span>':'')+'<span class="resultAddButtons"><button class="small quickAddWord" type="button" data-prefer="vocab" data-form="'+esc(item.word)+'" data-gloss="'+esc(item.gloss||'')+'">Add as duplicate/synonym to Vocabulary</button><button class="small quickAddWord" type="button" data-prefer="lex" data-form="'+esc(item.word)+'" data-gloss="'+esc(item.gloss||'')+'">Add as duplicate/synonym to Lexicon</button><button class="small quickAddWord" type="button" data-prefer="name" data-form="'+esc(item.word)+'" data-gloss="'+esc(item.gloss||'')+'">Add as duplicate/variant to Names</button></span></div></article>';
    }).join('');
  ...
  function doExport(){ var M=core(); if(!M){ status('MorfCore did not load.', 'error'); return false; } var st=readDomIntoState(safeState()); window.__MorfSafeState=st; mutateAppState(st); download('morf-3-4-5-settings.morf', M.exportState(st)); status('Exported settings file.','success'); return false; }
