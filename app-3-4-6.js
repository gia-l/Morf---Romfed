(function(){
  ...
  const STORE_KEY = 'morf-3-4-6-compat-settings';
...
Jord[a/y]n, Jordy = river child">${escapeHtml(M.nameEntriesToText ? M.nameEntriesToText(c.entries || []) : '')}</textarea></label>
  ...
  function exportSettings(){
    try {
      ...
      download('morf-3-4-6-settings.morf', json, 'application/json');
  ...
  async function copySettings(){
    try {
      ...
      download('morf-3-4-6-settings.morf', M.exportState(state), 'application/json');
    }
  }
  ...
  function bindSettings(){
    ...
    $('#copySettingsBtn').addEventListener('click', async () => {
      ...
      catch(err){ download('morf-3-4-6-settings.morf', M.exportState(state), 'application/json'); }
    });
