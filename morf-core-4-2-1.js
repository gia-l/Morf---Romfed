(function(root){
  ...
  function isStoredSegment(seg){
    return !!(seg && (seg.source === 'lexicon' || seg.source === 'vocabulary' || seg.source === 'name'));
  }

  function collapseGeneratedSegmentsForDisplay(segs, finalWord=''){
    const raw = Array.isArray(segs) ? segs : [];
    const joined = raw.map(s => String((s && s.form) || '')).join('');
    const knownCount = raw.filter(isStoredSegment).length;
    if(!raw.length){
      return finalWord ? [{ form: String(finalWord), cat: 'generated', source: 'generated', gloss: '?' }] : [];
    }
    if(!knownCount){
      const form = String(finalWord || joined || '').trim();
      return form ? [{ form, cat: 'generated', source: 'generated', gloss: '?' }] : [];
    }
    const out = [];
    let buffer = '';
    const flush = () => {
      if(buffer){
        out.push({ form: buffer, cat: 'generated', source: 'generated', gloss: '?' });
        buffer = '';
      }
    };
    ...
    flush();
    return out.length ? out : (finalWord ? [{ form: String(finalWord), cat: 'generated', source: 'generated', gloss: '?' }] : []);
  }
  ...
  function generateWords(state, options={}){
    ...
    while(results.length < target && stats.attempts < maxAttempts){
      ...
      try {
        ...
        const displaySegs = collapseGeneratedSegmentsForDisplay(segs, word);
        results.push({ word, gloss, segs: displaySegs, rawSegs: segs, analysis, rewrites: rewritten.fired, adjustments: adjusted.changed });
      } catch(err){
        stats.failed++;
        if(stats.errors.length < 10) stats.errors.push(err.message || String(err));
      }
  ...
  const api = {
    ...
    analyzeNameLiteral,
    glossForSegments,
    collapseGeneratedSegmentsForDisplay,
    parseRewriteRules,
    applyRewriteRules,
    parseForbiddenRules,
    violatesForbidden,
