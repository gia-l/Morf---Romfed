(function(){
  ...
  const STORE_KEY = 'morf-4-2-1-settings';
...
Jord[a/y]n, Jordy = river child">${escapeHtml(M.nameEntriesToText ? M.nameEntriesToText(c.entries || []) : '')}</textarea></label>
  ...
  function exportSettings(){
    try {
      ...
      download('morf-4-2-1-settings.morf', json, 'application/json');
  ...
  async function copySettings(){
    try {
      ...
      download('morf-4-2-1-settings.morf', M.exportState(state), 'application/json');
    }
  }
  ...
  function bindSettings(){
    ...
    $('#copySettingsBtn').addEventListener('click', async () => {
      ...
      catch(err){ download('morf-4-2-1-settings.morf', M.exportState(state), 'application/json'); }
    });
